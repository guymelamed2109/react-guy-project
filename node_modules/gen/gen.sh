...
create () {
  curl -X POST -H "Authorization: token $token" "https://api.github.com/user/repos" -d "{\"name\":\"$name\"}"
}
...
delete () {
  ...
  curl -X DELETE -H "Authorization: token $token" "https://api.github.com/repos/$username/$name"
}
